...
function ray_color(r: Ray, world: HittableList, depth: number): Color {
  ...
  if (world.hit(r, 0, Infinity, rec)) {
    let s = rec.p
      .add(rec.normal)
      .add(Vec3.random_unit_sphere_vector().unit_vector);
...
function antialiasing(
  ...
  for (let s = 0; s < samplesPerPixel; ++s) {
    let u = (x + Math.random()) / (w - 1);
    let v = (y + Math.random()) / (h - 1);
    const r = cam.getRay(u, v);
    color = color.add(ray_color(r, world, 50));
  }
  color.e = color.e.map((value) => Math.sqrt(value / samplesPerPixel));
  return color.uint8_color;
}
interface Props {
  imgWidth: number;
  imgHeight: number;
